...
class ValidatorRequest
{
    ...
    public function __construct($lang = 'PE')
    {
        if ($lang) {
    ...
    public function get_error()
    {
        ...
        $withData = $request->get_readable_errors(false);
        return (!empty($withData)?(object)array("data"=>$withData):(object)array("data"=>array()));
        
    }

    public static function is_correct( $data =array(),$validators =array())
    {
        $request = self::get_instance();

        $request->validationRules($validators);

        if ($request->run($data) === false) {
            return $request->get_readable_errors(false);
        } else {
            return true;
        }
    }

    public static function filter_input($data = array() , $filters  =array())
    {
        $request = self::get_instance();

        return $request->filter($data, $filters);
    }
    ...
    public function run( $data, $check_fields = false)
    {
    ...
    public function sanitizeValues( $input,  $fields = array(), $utf8_encode = true)
    {
    ...
    public function get_readable_errors($to_string = false)
    {
        if (empty($this->errors)) {
            return ($to_string) ? null : array();
        }
    ...
    public function get_errors_array($to_string = null)
    {
        if (empty($this->errors)) {
            return ($to_string) ? null : array();
        }
