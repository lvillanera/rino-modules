...
class Tpl {
	...
	protected static $conf = array(
		...
		'tags' => array(
			...
			'loop_for'=>array('({loop_for.*?})','/{loop_for="(\$.*)"}/'),
			'end_for'=>array('({\/loop_for})','/{\/loop_for}/')
		),
		'sandbox' => false,
	 ...
	 */
	public static function registerPlugin(Rino\Rain\Tpl\IPlugin $plugin, $name = '') {
		if ('' === $name) {
	...
	protected function checkTemplate($template) {
		...
		if (strpos($template, '/') === false) {
			...
			// die("plantilla");
			$tempFile = WEB_ROOT . DS . str_replace('/', DS, $template);

			$templateName = basename($tempFile);
			$templateBasedir = strpos($tempFile, DIRECTORY_SEPARATOR) ? dirname($tempFile) . DIRECTORY_SEPARATOR : null;
			$templateDirectory = dirname($tempFile); // static::$conf['tpl_dir'] . $templateBasedir;
			$templateFilepath = $tempFile; // $templateDirectory . $templateName . '.' . static::$conf['tpl_ext'];
			$parsedTemplateFilepath = static::$conf['cache_dir'] . $templateName . "." . md5($templateDirectory . serialize(static::$conf['checksum'])) . '.rtpl.php';
	...
	protected function compileFile($templateName, $templateBasedir, $templateDirectory, $templateFilepath, $parsedTemplateFilepath) {
		...
		if (flock($fp, LOCK_SH)) {
			...
			// read the file
			$code = fread($fp, filesize($templateFilepath));
			...
			// no mostrar este error al usuario :P
		...
		//read all parsed code
		{
			foreach ($codeSplit as $html) {
				...
				elseif (preg_match($tagMatch['include'], $html, $matches)) {
					...
					//$temp=APP_ROOT.DS.str_replace('/',DS,$this->varReplace($matches[1], $loopLevel));
				...
				elseif(preg_match($tagMatch['loop_for'], $html,$matches))
				{
					// $loopLevel++;
					// $var = $this->varReplace($matches['variable'], $loopLevel - 1, $escape = FALSE);
					// print_r($matches);
					$parsedCode .='<?php for('.$matches[1] .'){?>';
				}

				//close loop tag
				elseif (preg_match($tagMatch['end_for'], $html)) {
				...
				elseif(preg_match($tagMatch['vars'], $html,$matches))
				{
					$NewVariable = $matches[1];

					$parsedCode .= "<?php " . $this->varReplace($NewVariable, $loopLevel, $escape = TRUE, $echo = false) . "; ?>";
